        ...
        <scroll-view :showsVerticalScrollIndicator="false">
            <view class="content">
                <view>
                    ...
                    <text-input class="box text-input border" v-model="title" :placeholder="$t('title')"/>
                </view>
                <touchable-opacity class="marT10 border text-input center" v-if="(templateOptions && Object.keys(templateOptions).length >0)" :on-press="()=>{showTemplate = true}">
                    <text v-if="!showTemplate">{{$i18n.t('autolive.react.step1')}}</text>
                    <nb-picker v-if="showTemplate"
                        mode="dropdown"
                        :selectedValue="template"
                        :onValueChange="onChangeTemplate">
                        <item v-for="(item,id) in templateOptions" :key="id" :label="item.title" :value="id" />
                    </nb-picker>
                </touchable-opacity>
                <view class="marT10 box border">
                    <text class="label h4">{{$i18n.t('autolive.guess_song.intro_game')}}</text>
                    <view class="border">
                        <image
                            :source="require('../../../../assets/images/guess_song.jpg')"
    ...
    export default {
            ...
            onPressNext() {
                if (this.template && this.templateOptions[this.template]) {
                    this.dataStream.setting={
                        videos : this.templateOptions[this.template].videos,
                    }
                }
                this.dataStream.title = this.title;
                this.dataStream.type="guess-song";
                this.navigation.navigate("GuessNameSongStep2", {
        ...
        mounted() {
            ...
            db.collection("user_saved").where("gs_uid", "==", this.$store.state.userInfo.id)
                ...
                console.log('song-guess -step 1: this.templateOptions',this.templateOptions)
